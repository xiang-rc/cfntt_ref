...
def DIF_INTT(a,omega):
    ...
    for i in range(log_n):
        ...
        for k in range(int(n/(4*J))):
            ...
            r = r - 1
            for j in range(J):
                t0 = op21(a[k*4*J+j] + a[k*4*J+j+2*J]) % q
                t1 = (op21(-a[k*4*J+j] + a[k*4*J+j+2*J])*w4[1][1]) % q
                t2 = op21(a[k*4*J+j+J] + a[k*4*J+j+3*J]) % q
                t3 = (op21(-a[k*4*J+j+J] + a[k*4*J+j+3*J])) % q
                a[k*4*J+j] = op21(t0 + t2) % q
